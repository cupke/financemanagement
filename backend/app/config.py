...
class Settings(BaseSettings):
      ...
      # Время жизни access-токена. 7 дней (10080 мин) — для self-hosted личного
      # приложения это удобно (не выкидывает каждый час). Токен stateless, без
      # refresh-механизма, поэтому короткий срок раздражал бы. Переопределяется
      # через env JWT_ACCESS_TOKEN_EXPIRES_MINUTES (например, 43200 = 30 дней).
      # Полноценные refresh-токены и инвалидация сессий — в «перспективах развития».
      jwt_access_token_expires_minutes: int = Field(default=10080)
