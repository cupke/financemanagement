...
async def export_transactions_csv(
    ...
    Поля: дата, тип, сумма, валюта, счёт, категория, счёт-получатель,
    сумма зачисления (для кросс-валютных переводов), заметка.
    ...
    for tx in transactions:
        writer.writerow([
            ...
            # Сумма зачисления на счёт-получатель в его валюте — только у
            # кросс-валютного перевода. Пусто для обычных операций и
            # одновалютных переводов. Без неё выгрузка такого перевода была бы
            # неполной (терялась бы сумма в валюте получателя).
            f"{tx.target_amount:.2f}" if tx.target_amount is not None else "",
            tx.note or "",
        ])
